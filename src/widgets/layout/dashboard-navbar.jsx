...
export function DashboardNavbar() {
  ...
  function flattenTree(tree) {
    return tree.flatMap((node) => [
      node, // Include the current node in the flattened result
      ...(node.children ? flattenTree(node.children) : []), // Recursively flatten children, if any
    ])
  }
  const { setToken, setUserName, setStaffId } = userStore((state) => state)

  const title = flattenTree(routes?.[0]?.pages)?.find((i) => i.path === '/' + page)
        ...
        <div className="capitalize">
          <Breadcrumbs className={`bg-transparent p-0 transition-all ${fixedNavbar ? 'mt-1' : ''}`}>
            <Link to={`/${layout}`}>
              ...
              >
                {layout === 'dashboard' ? 'Панели управления' : 'Аутентификация'}
              </Typography>
            </Link>
